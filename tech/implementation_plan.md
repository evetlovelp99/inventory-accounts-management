...
- [ ] 0.8 后端：实现全局异常处理器（`GlobalExceptionHandler`），统一返回 `{ code, message, data }` 结构
...
- [ ] 1.4 后端：实现 `POST /api/inventory/inbound`（含事务：插入 inbound_records，更新无需改 remaining_qty — 初始值等于 quantity；记录 operation_log）
...
- [ ] 1.6 后端：实现 `POST /api/inventory/outbound`
  ...
  - 并发冲突时返回 409
- [ ] 1.7 后端：实现 `GET /api/inventory/stock`（聚合查询每个产品的 `SUM(remaining_qty)`）
...
- [ ] 1.17 前端：实现 `ProductsPage`（列表 + 新增/编辑 Modal + 停用确认）
- [ ] 1.18 前端：实现 `SuppliersPage`
- [ ] 1.19 前端：实现 `CustomersPage`

### 前端 — 仓库页面

- [ ] 1.20 前端：实现 `InboundEntryPage`（产品/供应商下拉读接口，自动计算总金额，提交后 Toast 反馈）
- [ ] 1.21 前端：实现 `OutboundEntryPage`（先选产品 → 调批次接口 → BatchRow 列表；汇总计算区实时更新；提交含 createReceivable 选项）
...
- [ ] 2.1 后端：实现 `GET /api/accounts/receivable`（按客户聚合，含账龄天数计算）
- [ ] 2.2 后端：实现 `GET /api/accounts/receivable/detail`（单客户明细 + 还款流水）
